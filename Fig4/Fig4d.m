...
% frequency
eeg_recording_start_datetime_str = '2023-12-11 12:25:00';
eeg_parquet_file_path = 'Resaved-biov-2023-12-11T12_25_00-06_00-2023-12-12T12_25_00-06_00-EEGN03.parquet'; % << USER: SET EEG PARQUET FILE PATH
...
imu_y_col_name = 'y';                
imu_z_col_name = 'z';                      
imu_recording_start_datetime_str = '2023-12-11 12:25:00'; 

% Time window for plotting
plot_window_start_datetime_str = '2023-12-12 02:35:00';
plot_window_end_datetime_str   = '2023-12-12 02:47:00'; 

% Seizure Information
seizure_onset_datetime_strs = {'2023-12-12 02:40:00'}; 
seizure_description_text = 'Right Event Example';
...
win_len_sec = 1.0;             % Window length for STFT (1.0s or 1.5s can be good for freq evolution)
...
figure_position = [100, 100, 1000, 350]; % Figure size for spectrogram plot
...
%% Time alignment and time window implementation
...
fig_handle = figure('Color','w'); % Set figure background to white
...
if ~isempty(P_db) && ~isempty(T_spec_datetime) && ~isempty(F_spec)
    ...
    % Set X-axis ticks to appear every 2 minutes
    t_start = effective_plot_start_dt;
    t_end = effective_plot_end_dt;
    
    % Round the start time down to the nearest minute for a clean start
    t_start_rounded = dateshift(t_start, 'start', 'minute');
    
    % Create a vector of datetimes every 2 minutes
    tick_vector = t_start_rounded : minutes(2) : (t_end + minutes(2));
    
    % Filter to only include ticks within the plot's time limits
    final_ticks = tick_vector(tick_vector >= t_start & tick_vector <= t_end);

    % If the plot is too short to contain any ticks, add one at the start
    if isempty(final_ticks)
        final_ticks = t_start;
    end
    
    % Apply the custom ticks to the axis
    ax_spec.XTick = final_ticks;
    
    % Determine the best time format and apply it to our custom ticks
    plot_duration_hours = hours(t_end - t_start);
    ...
    datetick(ax_spec, 'x', tick_format, 'keepticks'); % Use 'keepticks'
...
% 
%     % Save .png file
%     png_filename_png_str = [plot_filename_no_ext_str, '.png'];
%     fprintf('Saving high-resolution PNG to: %s\n', png_filename_png_str);
%     try
%       exportgraphics(fig_handle, png_filename_png_str, 'Resolution', 300); fprintf('High-resolution PNG saved.\n');
%     catch ME_export
...
%         try
%             saveas(fig_handle, png_filename_png_str); fprintf('PNG saved with saveas.\n');
%         catch ME_saveas
